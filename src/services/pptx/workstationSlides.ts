...
export interface LightingSlideParams {
...
export async function generateLightingPhotosSlide(
  ...
  // Title
  slide.addText(isZh ? '打光效果' : 'Lighting Effect', {
    x: 0.4, y: 0.05, w: 7.5, h: 0.38,
  ...
  if (params?.selectedLight) {
    paramItems.push({ label: isZh ? '光源' : 'Light', value: params.selectedLight });
  }
  if (params?.selectedCamera) {
    paramItems.push({ label: isZh ? '相机' : 'Camera', value: params.selectedCamera });
  }
  if (params?.selectedLens) {
    paramItems.push({ label: isZh ? '镜头' : 'Lens', value: params.selectedLens });
  }
  if (params?.triggerType) {
    const triggerLabel = TRIGGER_LABELS[params.triggerType]?.[isZh ? 'zh' : 'en'] || params.triggerType;
    paramItems.push({ label: isZh ? '触发方式' : 'Trigger', value: triggerLabel });
  }

  let photoStartY = paramY + 0.05;

  if (paramItems.length > 0) {
    // Draw parameter bar background
    slide.addShape('rect', {
      x: contentX, y: paramY, w: contentW, h: paramH,
      fill: { color: COLORS.lightGray },
      rectRadius: 0.05,
    });

    // Render parameter items evenly spaced
    const itemW = contentW / paramItems.length;
    paramItems.forEach((item, idx) => {
      const itemX = contentX + idx * itemW;
      slide.addText([
        { text: `${item.label}: `, options: { fontSize: 8, fontFace: FONTS.body, color: COLORS.textSecondary, bold: true } },
        { text: item.value, options: { fontSize: 8, fontFace: FONTS.body, color: COLORS.textPrimary } },
      ], {
        x: itemX + 0.1, y: paramY, w: itemW - 0.2, h: paramH,
        valign: 'middle',
      });
      ...
      if (idx > 0) {
        slide.addShape('line', {
          x: itemX, y: paramY + 0.06, w: 0, h: paramH - 0.12,
          line: { color: COLORS.border, width: 0.5 },
        });
      }
    });

    photoStartY = paramY + paramH + 0.1;
  }

  // === Photos area ===
  const photoEndY = SLIDE_LAYOUT.contentBottom;
  const contentH = photoEndY - photoStartY;
  const remarkH = 0.25;
  ...
  if (photos.length === 1) {
    ...
    try {
      ...
      slide.addImage({ data: dataUri, x: imgX, y: photoStartY, w: imgW, h: imgH, sizing: { type: 'contain', w: imgW, h: imgH } });
    } catch {
      slide.addShape('rect', { x: imgX, y: photoStartY, w: imgW, h: imgH, fill: { color: COLORS.lightGray }, line: { color: COLORS.border, width: 0.5 } });
      slide.addText(isZh ? '图片加载失败' : 'Image load failed', { x: imgX, y: photoStartY + imgH / 2 - 0.15, w: imgW, h: 0.3, fontSize: 10, color: COLORS.textSecondary, align: 'center' });
    }
    if (photos[0].remark) {
      slide.addText(photos[0].remark, { x: imgX, y: photoStartY + imgH + 0.03, w: imgW, h: remarkH, fontSize: 9, fontFace: FONTS.body, color: COLORS.textSecondary, align: 'center' });
    }
  } else {
    const cols = 2;
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      const y = photoStartY + row * (cellH + gap);
